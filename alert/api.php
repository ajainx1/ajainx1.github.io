...
function get_active_viewers_count() {
    ...
    // Explicitly hide specific IPs from the active connections list as requested
    $hidden_ips = ['10.133.0.51'];
    $unique_ips = array_filter($unique_ips, function($ip) use ($hidden_ips) {
        return !in_array($ip, $hidden_ips);
    });
...
function get_last_log_lines($n = 35, $filter = null) {
    ...
    // Highly optimized block-reading tail function to prevent IIS deadlocks on huge files
    $chunk_size = 8192; // 8KB chunks
    $pos = $file_size;
    $lines = [];
    $data = "";
    
    // Only search the last 1MB maximum to prevent infinite hangs
    $bytes_read = 0;
    $max_bytes = 1048576; 
    
    while ($pos > 0 && count($lines) < $n && $bytes_read < $max_bytes) {
        ...
        $chunk_lines = explode("\n", $data);
        // The first element might be an incomplete line, keep it in $data
        $data = array_shift($chunk_lines);
        
        // Process lines backwards
        for ($i = count($chunk_lines) - 1; $i >= 0; $i--) {
            $trimmed = trim($chunk_lines[$i]);
            if ($trimmed !== "") {
...
function send_whatsapp_alert($ip, $description, $event_type, $is_test = false) {
    ...
    // Rule: Suppress recovery (UP) alerts. Only DOWN, ON BATTERY, and POWER RECOVERY alerts are sent!
    if ($event_type !== 'DOWN' && $event_type !== 'ON BATTERY' && $event_type !== 'POWER RECOVERY') {
...
function run_parallel_ping_cycle($force = false) {
    // Process locking to prevent concurrent runs stacking up
    $lock_file = __DIR__ . DIRECTORY_SEPARATOR . 'ping.lock';
    ...
    // Launch all pings in parallel using proc_open batching (no temporary disk files, no start /B limits)
    $results = [];
    $up_count = 0;
    $down_count = 0;
    ...
    $batches = array_chunk($hosts, 50, true);

    foreach ($batches as $batch) {
        $procs = [];
        foreach ($batch as $idx => $host) {
            $ip = $host['ip'];
            $cmd = "ping -n 1 -w " . (PING_TIMEOUT_SEC * 1000) . " " . escapeshellarg($ip);
        ...
        foreach ($batch as $idx => $host) {
            ...
            $stdout = "Process creation failed.";
            $is_up = false;
            $latency = null;

            if (isset($procs[$idx]) && $procs[$idx] !== null) {
                $item = $procs[$idx];
                $stdout = stream_get_contents($item['stdout']);
                fclose($item['stdout']);
                fclose($item['stderr']);
        ...
        $fail_counters = $state['fail_counters'] ?? [];
        $current_fail_count = $fail_counters[$ip] ?? 0;

        if (!$is_up) {
            $current_fail_count++;
            $fail_counters[$ip] = $current_fail_count;
        } else {
            $fail_counters[$ip] = 0;
            $current_fail_count = 0;
        }
        $state['fail_counters'] = $fail_counters;

        // Anti-Flapping Hysteresis: Require 2 consecutive failures before declaring confirmed DOWN transition
        $prev_status = $prev_states[$ip] ?? 'UP';
        $confirmed_status = $prev_status;

        if (!$is_up && $current_fail_count >= 2) {
            $confirmed_status = 'DOWN';
        } elseif ($is_up) {
            $confirmed_status = 'UP';
        }

        $status_text = $confirmed_status;
        ...
        // Alert checking & logging (triggers ONLY on confirmed state transitions)
        if ($status_text !== $prev_status) {
            $prev_states[$ip] = $status_text;
            
            if ($status_text === 'DOWN') {
                write_log("WARNING", "[CONFIRMED OUTAGE] $desc ($ip) failed 2 consecutive cycles and transitioned to DOWN.");
                if (!$is_muted) {
                ...
                write_log("INFO", "[STATE CHANGE] $desc ($ip) has recovered and transitioned to UP.");
                if (!$is_muted) {
        ...
        // Synchronous HTTP polling removed. UPS data is injected via native_data.json.
    ...
    // Check for UPS Battery State transitions and trigger WhatsApp alerts
    ...
    if (file_exists($native_data_path)) {
        ...
        if ($native) {
            $ups_configs = [
                '10.133.15.42' => [
                    'name' => 'Legrand Main UPS',
                    'on_battery' => $native['ups1_on_battery'] ?? false,
                    'runtime' => $native['ups1_runtime'] ?? null
                ],
                '10.133.15.45' => [
                    'name' => 'Delta Backup UPS',
                    'on_battery' => $native['ups2_on_battery'] ?? false,
                    'runtime' => $native['ups2_runtime'] ?? null
                ]
            ];
            
            $prev_on_battery = $state['on_battery'] ?? [];
            
            foreach ($ups_configs as $ups_ip => $cfg) {
                $was_on_bat = $prev_on_battery[$ups_ip] ?? false;
                $is_on_bat = $cfg['on_battery'];
                
                if ($is_on_bat !== $was_on_bat) {
                    $prev_on_battery[$ups_ip] = $is_on_bat;
                    if ($is_on_bat) {
                        $rt_str = $cfg['runtime'] !== null ? " Estimated runtime: " . $cfg['runtime'] . " minutes." : "";
                        $msg = "[POWER OUTAGE] " . $cfg['name'] . " ($ups_ip) is running ON BATTERY!" . $rt_str;
                        write_log("CRITICAL", $msg);
                        send_whatsapp_alert($ups_ip, $msg, 'ON BATTERY');
                    } else {
                        $msg = "[POWER RECOVERY] " . $cfg['name'] . " ($ups_ip) has restored UTILITY POWER.";
                        write_log("INFO", $msg);
                        send_whatsapp_alert($ups_ip, $msg, 'POWER RECOVERY');
                    }
                }
            }
            $state['on_battery'] = $prev_on_battery;
        }
    }
...
switch ($endpoint) {
    ...
    case 'status':
        ...
        // Self-Healing Telemetry Engine: Asynchronously refresh telemetry data when cache > 30s without blocking HTTP response
        if (!$is_paused && (time() - $last_ping > 30 || isset($_GET['force']))) {
            $lock_file = __DIR__ . DIRECTORY_SEPARATOR . 'ping.lock';
            $should_spawn = true;
            if (file_exists($lock_file)) {
                $lock_time = intval(@file_get_contents($lock_file));
                if (time() - $lock_time < 20) {
                    $should_spawn = false;
                }
            }
            if ($should_spawn) {
                ...
                if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                    @pclose(@popen("start /B php \"" . __FILE__ . "\" --run_ping_cycle", "r"));
                } else {
                    @exec("php \"" . __FILE__ . "\" --run_ping_cycle > /dev/null 2>&1 &");
                }
            }
        }
                ...
                if ($native) {
                    ...
                    foreach ($data['hosts'] as &$host) {
                        if ($host['ip'] === "10.133.15.42") {
                            ...
                            $host['on_battery'] = $native['ups1_on_battery'] ?? false;
                            if (isset($native['ups1_runtime'])) {
                                $host['runtime'] = $native['ups1_runtime'];
                            }
                        }
                        if ($host['ip'] === "10.133.15.45") {
                            $host['battery_status'] = $native['ups2_bat'] ?? "N/A";
                            $host['temp'] = $native['ups2_tmp'] ?? "N/A";
                            $host['on_battery'] = $native['ups2_on_battery'] ?? false;
                            if (isset($native['ups2_runtime'])) {
                                $host['runtime'] = $native['ups2_runtime'];
                            }
                        }
                        // Inject PAC temperatures
                        if (isset($native['pac_temps']) && isset($native['pac_temps'][$host['ip']])) {
                            $host['temp'] = $native['pac_temps'][$host['ip']];
                        }
                    }
                }
            }
    ...
    case 'hosts/ping':
        ...
        if (!empty($state)) {
            ...
            // Process leak fix: Do not endlessly spawn native_poller.py here
            save_state($state);
        }
    ...
    case 'hosts/mute':
        if (!is_admin_authorized()) {
            http_response_code(403);
            echo json_encode(['status' => 'error', 'message' => 'Authorization Required: Administrative login required to mute network device alarms.']);
            break;
        }
        $input = json_decode(file_get_contents('php://input'), true);
        $ip = $input['ip'] ?? '';
    ...
    case 'hosts/mute_bulk':
        if (!is_admin_authorized()) {
            http_response_code(403);
            echo json_encode(['status' => 'error', 'message' => 'Authorization Required: Administrative login required to bulk mute network alarms.']);
            break;
        }
        $input = json_decode(file_get_contents('php://input'), true);
        $ips = $input['ips'] ?? [];
        $action = $input['action'] ?? 'mute'; // 'mute' or 'unmute'
        if (empty($ips)) {
            http_response_code(400);
            echo json_encode(['status' => 'error', 'message' => 'IP list is required.']);
            break;
        }
        ...
        if (!empty($state)) {
            $muted = $state['muted'] ?? [];
            if ($action === 'mute') {
                $muted = array_unique(array_merge($muted, $ips));
            } else {
                $muted = array_values(array_diff($muted, $ips));
            }
            $state['muted'] = $muted;
            save_state($state);
        }
        ...
        if (file_exists(MONITOR_DATA_FILE)) {
            ...
            foreach ($mdata['hosts'] as &$host) {
                if (in_array($host['ip'], $ips)) {
                    $host['muted'] = ($action === 'mute');
                }
            }
            file_put_contents(MONITOR_DATA_FILE, json_encode($mdata, JSON_PRETTY_PRINT));
        }
        
        echo json_encode(['status' => 'success', 'count' => count($ips), 'action' => $action]);
        break;
    ...
    case 'diagnostics/tracert':
        ...
        if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
            http_response_code(400);
            echo json_encode(['status' => 'error', 'message' => 'Invalid IPv4 target format.']);
            break;
        }
        $ip_client = $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1';
        $rate_dir = sys_get_temp_dir() . '/tracert_limits/';
        if (!is_dir($rate_dir)) @mkdir($rate_dir, 0755, true);
        $limit_file = $rate_dir . md5($ip_client) . '.txt';
        $last_time = file_exists($limit_file) ? intval(@file_get_contents($limit_file)) : 0;
        if (time() - $last_time < 5) {
            http_response_code(429);
            echo json_encode(['status' => 'error', 'message' => 'Rate limit exceeded: Please wait 5s between network trace requests.']);
            break;
        }
        @file_put_contents($limit_file, (string)time());
        $stdout = [];
        exec("tracert -d -h 8 -w 500 " . escapeshellarg($ip), $stdout);
    ...
    case 'trigger-active-outages':
        if (!is_admin_authorized()) {
            http_response_code(403);
            echo json_encode(['status' => 'error', 'message' => 'Authorization Required: Administrative login required to trigger manual broadcast alerts.']);
            break;
        }
        $state = load_state();
        if (!empty($state)) {
    ...
    case 'test-alert':
        if (!is_admin_authorized()) {
            http_response_code(403);
            echo json_encode(['status' => 'error', 'message' => 'Authorization Required: Administrative login required to send test alerts.']);
            break;
        }
        $state = load_state();
        if (!empty($state)) {
